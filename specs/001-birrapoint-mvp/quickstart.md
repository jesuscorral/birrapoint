...
| 2 | Wizard & drafts (US2) | Create competition, leave step 2 via "Save draft", reopen; separately, walk all 6 steps to completion (Basics → Details → Categories → Import beers → Import judges → Mesas) | Draft state persisted; wizard resumes with data intact; Next disabled until name/venue/dates valid; step 5's "Continuar" advances to step 6 (table setup, US5) rather than exiting the wizard; step 6's own action returns to the dashboard |
...
| 4 | Judge invitations (US4) | Bulk-add emails incl. one duplicate; then trigger "Notify judges" | Profiles + accounts created (no email yet), duplicate reported; triggering "Notify judges" delivers invitations, visible in Mailpit (`:8025`) |
...
| 13 | Competition selection (US13) | Log in as an organizer owning 2+ competitions in different states; select one, then separately start a new one; confirm-advance a `Draft` competition to `Active` from the dashboard; attempt advancing to `Finalized` with an open table | Dashboard lists all owned competitions with name/venue/dates/state; selecting one opens the wizard (`Draft`), the wizard's table step `?step=6` (`Active`) or the live monitor (`In Evaluation`/`Finalized`, which links to the read-only wizard — FR-061); "new competition" opens the wizard empty; zero owned competitions shows an empty state with a create action; confirmed advance moves the badge to `Active` with no reload; `Finalized` attempt with an open table is blocked and lists which table(s) |
| 14 | Judge roster import (US14) | In wizard step 5, upload `samples/judges-with-errors.xlsx` (a roster mixing valid rows, one row missing an email, and one duplicate email) | Row-level results; correction screen resolves the `Invalid` row; consolidation blocked until resolved; on consolidate, one account per unique email is created (verify via Keycloak admin or the judges list), duplicate reported, **no invitation email sent yet** (Mailpit stays empty for these addresses until "Notify judges" is triggered separately, scenario 4) |

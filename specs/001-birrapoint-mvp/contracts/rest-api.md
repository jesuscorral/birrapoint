...
| Method & Path | Role | Description |
|---|---|---|
| `GET /competitions/{id}/entries` | ORGANIZER | Every entry in the competition with its current table assignment — feeds the table-setup UI's "Unassigned" column (T048). `200` → `[{ id, blindCode, styleCode, styleName, abvLow, abvHigh, abvPercent, beerName, notValidForBos, tastingTableId, tastingTableName, competitionCategoryName, bjcpCategoryNumber, bjcpCategoryName }]`; `abvLow`/`abvHigh` are the BJCP style's declared ABV range (catalog-sourced, may both be `null` — some styles, e.g. historical ones, have no declared range), `abvPercent` is this entry's actual submitted ABV (required at import, never `null`); `tastingTableId`/`tastingTableName` are `null` until the entry is assigned via `POST`/`PUT .../tables`. `competitionCategoryName` is the organizer-defined category from wizard step 3 (`null` for entries created outside the import flow); `bjcpCategoryNumber`/`bjcpCategoryName` are the BJCP taxonomy's own category for `styleCode` (e.g. `"21"`/`"IPA"`), an independent axis from `competitionCategoryName`. They are typed nullable defensively but are not reachable as `null`: `BeerEntry.StyleCode` is a required, non-nullable FK to the catalog with `OnDelete(Restrict)`, so an entry whose style has no catalog row cannot exist — clients need not branch on it. Organizer-only, so unlike judge-facing DTOs this includes `beerName` (no BR-01 concern). |
...
| `GET /competitions/{id}/tables` | ORGANIZER | `200` → `[{ id, name, state, judges: [{ id, email, displayName }], samples: [{ beerEntryId, blindCode, styleCode, styleName, abvLow, abvHigh, abvPercent, notValidForBos, entryInstructions, competitionCategoryName, bjcpCategoryNumber, bjcpCategoryName }], progress: { submitted, total }, stats: { meanAbv, styleCount, styles } }]` (same `samples`/`stats` shape is reused by the `POST`/`PUT` responses below, alongside `bosFlaggedEntryIds`). `abvLow`/`abvHigh` per sample are the BJCP style's declared range (may both be `null`); `stats.meanAbv` is the average of the table's entries' actual `abvPercent` — not the styles' declared range — and is `null` only when the table has zero samples. `competitionCategoryName`/`bjcpCategoryNumber`/`bjcpCategoryName` per sample carry the same semantics as on `GET /competitions/{id}/entries` above. |
...
| `GET /me/tables/{tableId}/samples` | JUDGE | `[JudgeSampleDto]` = `{ beerEntryId, blindCode, styleCode, styleName, abvPercent, sequenceOrder?, evaluationStatus: "NotStarted" \| "Submitted" \| "PendingConsensus", entryInstructions? }` — `entryInstructions` is the one deliberate exception to the BR-01/FR-019 anonymity boundary alongside `blindCode`/`styleCode`; `abvPercent` is the beer's own attribute (not an entrant field), same value the organizer sees on `TableSampleDto`. |
| `GET /me/tables/{tableId}/judges` | JUDGE | Every other actively-assigned judge at the table (caller excluded), informational only: `[{ displayName, bjcpRank? }]`. Same 404 scoping as the endpoint above. |
...
| `POST /me/tables/{tableId}/evaluations` | JUDGE | Header `X-Idempotency-Key: {competitionId}:{tableId}:{judgeId}:{entryId}`. Body: `{ beerEntryId, scores: { aroma, appearance, flavor, mouthfeel, overall }, comments: { aroma, appearance, flavor, mouthfeel, overall }, descriptors?, feedback? }`. `descriptors`/`feedback` (FR-063, Session 2026-09-21) are optional structured tasting-sheet data — see data-model.md's `EvaluationDescriptorsDto` shape table — validated (range/closed-list, when supplied) but never required and never fed into `total`/the caps. Validation (FR-023/FR-025): caps 12/3/20/5/10, comments ≥ 20 chars → `400`; an out-of-range or off-catalog `descriptors` field also → `400`. Preconditions: competition `InEvaluation` (`409 invalid-state-transition`), order fixed (`409 order-not-fixed`), sample is next in sequence (`409 out-of-sequence`), table open (`409 table-closed`). Success → `201` `{ evaluationId, status: "Confirmed" \| "PendingConsensus", total, discrepancy? }`. Idempotent replay → `200` with the stored result (R-07). |
...
| `GET /competitions/{id}/entries/{entryId}/evaluations` | ORGANIZER | Audit drill-down, read-only: full evaluations incl. judge names + consolidated mean when table closed (FR-038, FR-042). Each item also carries `descriptors?`/`feedback?` (FR-063, Session 2026-09-21) — `null` when the judge left them blank. |
| `PUT /competitions/{id}/evaluations/{evaluationId}` | ORGANIZER | Post-close correction (FR-035). Body: same scores/comments shape as judge submission, plus the same optional `descriptors`/`feedback` (FR-063); same caps/length/range validation. `scores`/`comments` are required on every correction (unchanged); `descriptors`/`feedback` are **preserve-when-absent** — omitting either leaves the judge's existing value untouched, it does not clear it (a correction of just the scores must not silently wipe unrelated tasting-sheet data). Recomputes total + consolidated mean; writes AuditLog (author, timestamp, before/after — now including `descriptorsJson`/`feedbackComment`). Allowed regardless of table state; organizer-only. |

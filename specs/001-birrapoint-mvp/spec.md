...
- **FR-019**: Judge-facing views MUST identify samples exclusively by blind code and style/category; brewer name, beer name, brewery, origin, and registration metadata MUST never appear in any judge-facing screen, list, message, or notification. The one deliberate exception is the entrant-authored `entryInstructions` field (e.g. serving/tasting guidance some BJCP styles require) — permitted in judge-facing views only because the organizer reviews and, if necessary, edits or clears that text during import/consolidation to strip anything identifying before it becomes visible (Session 2026-08-02).

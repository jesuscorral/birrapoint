...
- [X] T136 [FR-064] Deploy pipeline `.github/workflows/deploy.yml` (`workflow_dispatch`: `version` + optional per-component overrides): a `resolve` job validates versions before approval; the `deploy` job runs in the `production` Environment (required reviewers), logs in to Azure with OIDC scoped to the application resource group and runs `infra/deploy.ps1 -AppsOnly` (no Terraform — infrastructure assumed to exist); `concurrency: deploy-production`; run summary of each app's serving revision/image; one-time setup (Docker Hub, Azure OIDC, environment, release bot vs. protected `main`) in `infra/github-actions-setup.md`

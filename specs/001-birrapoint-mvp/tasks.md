...
- [X] T019 [P] Frontend core auth: Keycloak initialization (PKCE), token interceptor, role guards, route shells `/organizer/**` and `/judge/**` in `frontend/src/app/core/auth/`
- [X] T020 [P] Frontend core services: typed API client with ProblemDetails→UI error mapping in `frontend/src/app/core/api/`, SignalR connection service with rejoin-on-reconnect in `frontend/src/app/core/realtime/`, Dexie database (`drafts`, `outbox` stores) in `frontend/src/app/core/offline/db.ts`
...
- [X] T045 [P] [US5] Unit tests: COI detection via owner/collaborator email match; BOS flag on any table membership; flag lift only before first submitted evaluation (FR-018 permanence) in `backend/tests/BirraPoint.Api.UnitTests/Tables/`
- [X] T046 [P] [US5] Contract tests: `409 conflict-of-interest` with `{judgeId, beerEntryIds}` payload and atomic rollback; success returns `bosFlaggedEntryIds`; entry-in-one-table uniqueness in `backend/tests/BirraPoint.Api.IntegrationTests/Tables/TablesApiTests.cs`

### Implementation for User Story 5

- [X] T047 [US5] Slices CreateTable, UpdateTable, ListTables with transactional COI validation + BOS flagging/unflagging + endpoints in `backend/src/BirraPoint.Api/Features/Tables/` — plus an addendum `GET /competitions/{id}/entries` endpoint (`Features/Tables/ListEntries.cs`), a real gap found mid-implementation: T048's "Unassigned" beer column had no endpoint anywhere in the contract to list a competition's entries
- [X] T048 [US5] Frontend table-management: `MesaCard` visual component rendering each table as a physical table board with judges as seats around it and assigned beer entries as tokens on top (per the organizer-supplied "Crear mesas" prototype — physical-table metaphor, not a plain list); per-table stats summary (mean ABV, style count, style list) always visible; an "Unassigned" source column (no table/seat iconography, plain judge/beer list) alongside one `MesaCard` per table; COI conflict dialog; BOS warning banner in `frontend/src/app/features/table-management/`
- [X] T048A [US5] Click-to-detail interactions: clicking a beer token opens a modal/panel with code, style, ABV, assigned table; clicking a judge seat opens a modal with name/initials, assigned table. Applies identically to judges/beers still in the "Unassigned" column (T048), not just once seated at a `MesaCard`. **Scope cut (user-approved)**: allergen/special-award beer badges and judge BJCP category/certification were dropped — zero backing in spec.md/data-model.md/contracts for any of them (an unbacked task-doc addition from a prior session); tracked as backlog below, needs its own spec amendment before it's buildable
- [X] T048B [US5] Drag & drop for judge/beer assignment (`@angular/cdk/drag-drop` — this phase establishes the pattern, since Phase 8/T053 lands later): drag from "Unassigned" into any `MesaCard`, and between `MesaCard`s; must not swallow the T048A click-to-detail interaction on the same element (click vs. drag-start disambiguation via a 6px pointer-movement threshold)
- [X] T048C [US5] Accessibility: seat (~38px) and beer-token (~64px) interactive targets meet a minimum hit-area size on desktop pointer input; this screen is desktop-only for now (no touch/mobile target-size variant needed yet) — a targeted check in addition to T089's broader axe-core sweep, since minimum-target-size is a WCAG 2.1 AA criterion axe-core does not reliably flag for custom drag-and-drop widgets
- [X] T049 [US5] E2E scenario 5 (extend with: beer/judge detail modal open+close, drag-and-drop assignment between "Unassigned" and a `MesaCard` and between two `MesaCard`s, and that a plain click on an already-assigned beer/judge opens its detail modal rather than starting a drag) in `frontend/e2e/us5-tables.spec.ts` — found and confirmed the fix for a real defect (BOS flag visual state not refreshing outside the mutated table) along the way
...
- [X] T055 [P] [US7] Unit tests: SubmitEvaluation validator (caps 12/3/20/5/10, comments ≥20), preconditions (state, order fixed, next-in-sequence), locked-on-submit rule in `backend/tests/BirraPoint.Api.UnitTests/Evaluations/SubmitEvaluationTests.cs`
- [X] T056 [P] [US7] Contract tests: `201` submit, idempotent replay `200` with stored result (`X-Idempotency-Key`), `409` order-not-fixed / out-of-sequence / invalid-state / evaluation-locked in `backend/tests/BirraPoint.Api.IntegrationTests/Evaluations/SubmitEvaluationApiTests.cs` — plus a real concurrent-race test (two simultaneous submits for the same judge+entry, exactly one row persists)
- [X] T057 [P] [US7] Jest unit tests: draft store writes ≤300 ms debounce contract, outbox replay (online event, app start, backoff, dedupe on replay-200) in `frontend/src/app/core/offline/sync.service.spec.ts`
- [X] T057B [P] [US7] Contract test: `GET /styles/{code}` shape (vital statistics + description sections), `404` for unknown code in `backend/tests/BirraPoint.Api.IntegrationTests/Catalog/GetStyleDetailTests.cs` (FR-049)

### Implementation for User Story 7

- [X] T058 [US7] Slice SubmitEvaluation: preconditions, idempotent create, `EvaluationCompleted` emit (organizer group), discrepancy detection hook point (activated in US11) in `backend/src/BirraPoint.Api/Features/Evaluations/SubmitEvaluation.cs` — idempotent replay via unique-constraint-catch (Npgsql `PostgresErrorCodes.UniqueViolation`), never pre-check-then-upsert (R-07 forbids it)
- [X] T059 [US7] Frontend evaluation sheet: five capped numeric sections + comment fields with live remaining-length, auto-computed read-only total, submit gating in `frontend/src/app/features/evaluation-sheet/`
- [X] T059B [US7] Slice GetStyleDetail (`GET /styles/{code}`, projects `BjcpStyle.DescriptionJson` + vital statistics) + endpoint in `backend/src/BirraPoint.Api/Features/Catalog/GetStyleDetail.cs` (FR-049)
- [X] T060 [US7] Frontend offline engine: draft persistence (≤300 ms after change), offline badge ("Offline mode — data protected locally"), outbox enqueue on submit, `SyncService` replay per R-08, and an explicit warning when local storage is unavailable/full (spec edge case — never fail silently) in `frontend/src/app/core/offline/sync.service.ts`
- [X] T060B [US7] Frontend style reference panel: collapsible read-only panel in the evaluation sheet showing the declared style's guide description (fetched via `GetStyleDetail`, cached for offline use), keyboard-accessible toggle (WCAG, Principle VIII) in `frontend/src/app/features/evaluation-sheet/style-reference/` (FR-049)
- [X] T061 [US7] E2E scenario 7: fill offline (Playwright `setOffline`), reload → draft intact, reconnect → exactly one server evaluation in `frontend/e2e/us7-offline.spec.ts` — literal `page.reload()` while offline is infeasible against the dev-mode harness (no service worker outside production builds, `no-cache` headers); substitutes `page.goBack()`/`page.goForward()` as the closest in-harness equivalent that still destroys/reconstructs the component and exercises real Dexie rehydration, documented inline; found and fixed a real bug along the way (evaluation-sheet.component.ts fell back to a load-error screen instead of the cached sample on a genuine offline restart)
...
- [X] T095 [P] Backend multi-stage Dockerfile (.NET SDK build → ASP.NET runtime) in `backend/src/BirraPoint.Api/Dockerfile` and frontend multi-stage Dockerfile (Node build → Nginx Alpine) in `frontend/Dockerfile` + `frontend/nginx/` (`default.conf.template`, `40-runtime-config.sh`), plus a production Keycloak image in `infra/keycloak/Dockerfile`; verify no secrets or environment-specific config are baked into either image (FR-043)
- [X] T096 Terraform deployment model: `infra/terraform/` (remote state in Azure Storage) provisioning the ACA environment, container apps for frontend (external ingress), backend, and Keycloak (realm import); a build/push step (CI or `docker push`, wrapped by `infra/deploy.ps1`) publishes images to Docker Hub ahead of `terraform apply`; all configuration/secrets injected via env vars/secrets, including the Neon connection string (FR-045/FR-046)
- [X] T097 Neon PostgreSQL provisioning: project/branch created (via Terraform's Neon provider or a documented manual step, per the plan's decision), pooled connection string wired into the backend Container App as a secret; document Neon's own point-in-time recovery in place of a self-managed backup/restore procedure (FR-047)
...
- [ ] T129 DispatchJob safe under overlapping API revisions (PR #46 review M3): ACA `Single` revision mode still runs the old and new revision side by side during a rollout, and `DispatchWorker.ResumeInterruptedJobsAsync` resets `Running` jobs the other replica is still executing while pending jobs have no atomic claim — duplicate result emails/PDFs possible. Claim jobs atomically (`FOR UPDATE SKIP LOCKED` + lease column, migration) and resume only expired leases; integration test with two concurrent workers in `backend/tests/BirraPoint.Api.IntegrationTests/Dispatch/`
...
- [X] T134 [FR-064] Decouple image deployment from infrastructure (option C, user decision 2026-09-26; ADR-0018): Terraform takes per-image references (`api_image`/`web_image`/`keycloak_image`, replacing `image_tag`) used only when a Container App is first created (`lifecycle.ignore_changes` on the container image) plus a mandatory per-apply `revision_suffix`; `infra/deploy.ps1` stops building/pushing, accepts an optional version per image (`-ApiVersion`/`-WebVersion`/`-KeycloakVersion`: `X.Y.Z` → `<ns>/birrapoint-*-release:X.Y.Z`, omitted → `<ns>/birrapoint-*:latest`), verifies the images exist on Docker Hub (authenticated when `DOCKERHUB_*` are set), runs `terraform apply` with a fresh `infra-<timestamp>` suffix, then rolls each app with `az containerapp update` (keycloak → api → web), deciding from the serving revision and polling the created revision until healthy; `-AppsOnly` skips state bootstrap and Terraform; `-WhatIf` changes nothing; Pester tests for the resolution/rollout logic; `.github/workflows/infra.yml` (Terraform fmt/validate + Pester, only on `infra/**` changes) — `ci.yml` has nothing Terraform-related
- [X] T135 [FR-064] Release pipeline `.github/workflows/release.yml` (`workflow_dispatch` from `main` only, inputs `ref` — `main`, `hotfix/*` or a `v*` tag — and `bump`; ADR-0019; an image already pushed for the same SHA is reused on re-run): reads `version.txt` from `main` (`.github/scripts/version.sh`, tested by `version.test.sh`), fails if tag/images for that version exist (`dockerhub-tag-exists.sh`), runs the reusable `quality-gates.yml` (extracted from `ci.yml`) on the resolved SHA, pushes only to `birrapoint-*-release` repos (re-checking immutability before each push), tags `vX.Y.Z`, creates the GitHub Release with image digests, advances `version.txt` on `main` with `GITHUB_TOKEN` and `[skip ci]`; idempotent finalize steps make "Re-run failed jobs" safe; `.github/workflows/workflows.yml` runs actionlint + the script tests
- [X] T136 [FR-064] Deploy pipeline `.github/workflows/deploy.yml` (`workflow_dispatch`: `version` + optional per-component overrides): dispatchable from `main` only; a `resolve` job validates versions and checks the release images exist before approval; the `deploy` job runs in the `production` Environment (required reviewers, deployment branches restricted to `main`, optional read-only Docker Hub token), logs in to Azure with OIDC scoped to the application resource group and runs `infra/deploy.ps1 -AppsOnly` (no Terraform — infrastructure assumed to exist); `concurrency: deploy-production`; run summary of each app's serving revision/image; one-time setup (Docker Hub, Azure OIDC, environment, release bot vs. protected `main`) in `infra/github-actions-setup.md`
- [ ] T137 [FR-064] CI/CD validation and documentation: validate ADR-0018's assumptions against a real deployment (revision-suffix behaviour, health semantics without probes and with the web app scaled to zero, and "roll out a release, then change an env var through `deploy.ps1`" succeeding without reverting the image) and amend ADR-0018 with the findings; `Docs/arquitectura_viva.md`, `infra/terraform/README.md`, `CLAUDE.md` Commands; run a first `0.1.0` release and production deploy. If CI becomes a required status check on a protected `main`, handle path-ignored (docs-only) PRs, which never produce the check (PR #48 review M5)
- [X] T138 Integration-test flakiness (PR #48 review L5): the flaky test was `DispatchApiTests.Retry_dispatch_resets_a_failed_job_and_it_gets_reprocessed_by_the_safety_net_poll` (failed in CI on PRs #50/#52 and on main). Root cause: a race in the test's assertion, not in the product — after `RetryDispatch` resets the job to Pending, the DispatchWorker's safety-net poll can pick it up and mark it Running before the test reads the status, and the test only accepted Pending/Completed. Fixed by asserting the retry's actual contract (no longer Failed: Pending, Running or Completed; error cleared); reproduced red by seeding the Running state, no production change, no retries
...
- [X] T140 [FR-045] Teardown script `infra/teardown.ps1` (user request 2026-09-26): removes every billed Azure resource (`terraform destroy -target=azurerm_resource_group.main`) while keeping the free Neon project and the Terraform state (which holds the Neon project and the generated secrets, so a redeploy reconnects with matching secrets); `-IncludeNeon` wipes everything incl. state and local `.terraform`; typed confirmation(s), `-WhatIf`, idempotent direct sweep when destroy fails or state is missing (`az group delete`, Neon API exact-name match), final verification; Log Analytics purged on destroy (`permanently_delete_on_destroy`); pure logic in `infra/Teardown.psm1` with Pester tests
...
## Phase 18: Competition Categories & Allowed Styles (FR-052, added 2026-07-30)

**Goal**: Wizard step 3 — organizer defines free-text categories and assigns a subset of BJCP
styles to each, persisted per competition and enforced during entry import (T106-T108) including
after the organizer returns to a pending import having edited categories mid-flow (T109)

**Independent Test**: create a competition, reach step 3, add ≥1 category, assign ≥1 style, save
— `GET /competitions/{id}/categories` reflects it; re-editing fully replaces the previous set

- [X] T104 [FR-052] Backend: `CompetitionCategory`/`CompetitionCategoryStyle` domain entities + EF configurations (unique `(CompetitionId, Name)` and `(CompetitionId, StyleCode)` indexes — the latter enforces "at most one category per style per competition" at the DB level) + migration; `GET`/`PUT /competitions/{id}/categories` (`Features/Competitions/GetCompetitionCategories.cs`, `SetCompetitionCategories.cs`) — same ownership + Draft/Active state-gate as `UpdateCompetition`; full-replace PUT semantics; FluentValidation (non-empty categories, ≥1 style assigned overall, unique names, no style repeated across categories, all style codes must exist in `BjcpStyles`). Unit tests (validator) + integration tests (roundtrip, non-owner 404, `Finalized` 409, duplicate/unknown style code 400, re-PUT fully replaces)
- [X] T105 [FR-052] Frontend: `features/competition-wizard/steps/categories-step.component.ts` — new terminal wizard step (stepper widened to 3); lists organizer categories (add/rename/remove), full BJCP catalog grouped by its own official category for browsing, per-style assignment via select (enforces the one-category-per-style rule client-side too), "Guardar y finalizar" gated on ≥1 category with ≥1 style, same "Volver al listado" save-or-discard confirm dialog as `basics-step.component.ts`. `details-step.component.ts`'s save action reverted to emit-and-let-parent-advance (step 3 is now the terminal step, not step 2). `CatalogApiService.getStyles()` and `CompetitionsApiService.getCategories()`/`setCategories()` added. Jest specs alongside for all of the above
- [X] T106 [FR-053] Backend: wire the FR-052 allow-list into `Features/Import` — new `ImportRowStatus.CategoryStyleMismatch`; `WorkbookParser.ParseRow` cross-checks the resolved category/style pair against `CompetitionCategoryStyle` (via a new `CategoryStyleCatalogEntry` projection loaded in `UploadImport.cs`); `EditImportRow.cs`'s completeness check does the same. Unit tests (`WorkbookParserTests`, `EditImportRowCommandValidatorTests`) + integration coverage in `ImportApiTests.cs`
- [X] T107 [FR-053] Backend: `ConsolidateImport.cs`'s unresolved-rows gate (`DomainErrorType.UnresolvedImportRows`) extended to also block on `CategoryStyleMismatch`
...
- [X] T124 [FR-016] Wizard step 6 ("Mesas") UI/UX pass. Backend (no migration — `BeerEntry.CompetitionCategoryId` already existed but was never projected): `TableSampleDto`/`EntryDto` gain `CompetitionCategoryName` (the organizer-defined wizard-step-3 category) plus `BjcpCategoryNumber`/`BjcpCategoryName` (the BJCP taxonomy's own, independent axis) in `Features/Tables/TableDtos.cs`/`ListEntries.cs`, joined in `TableProjector.cs`/`ListEntries.cs`; `contracts/rest-api.md` amended for both DTOs (additive, non-breaking). Frontend: `beer-token.component.ts` gains a `full`/`mini` variant — `full` (the "Unassigned" panel) renders a beer-glass icon plus style, competition category, BJCP category and real ABV%; `mini` (seated on a MesaCard) stays a compact code + ABV pill. `judge-seat.component.ts` shows the full display name under the initials avatar (`showName`, default on) — initials alone could not tell two judges apart while seating them. `mesa-card.component.ts` replaces the oval physical-table board with a compact card: "Cervezas" and "Media ABV" get primary visual weight, style names move to their own line, and both drop zones carry a visible empty-state hint. `table-board.component.ts` becomes a two-column grid (sticky source panel + `repeat(auto-fill, minmax(15rem, 1fr))` tables grid, single column under 900px) so every table stays on screen as a drop target while dragging, plus a first-run empty state. `table-detail-modal.component.ts` surfaces both category axes. `--spacing-1` added to `styles.css` for the new compact density. Every E2E-locked contract preserved byte-for-byte: the `Beer {code} — view details` / `Judge {name} — view details` accessible names, the `beer-token--bos-flagged` class, the `judges-*`/`beers-*` drop-list ids, `dd[data-stat="judges"|"beers"]`, and the English `New table name` / `Add table` form strings (nine E2E specs). WCAG 1.3.1/1.4.1 kept by moving the new visual detail into an `aria-describedby` description rather than into the accessible name. Jest specs alongside for every file above
- [X] T125 [FR-016] Wizard shell + step-6 layout, desktop-first. `competition-wizard.component.ts`'s `.wizard-container` goes from `max-width: 40rem` to `min(100%, 96rem)`; the form steps keep a readable measure via their own `.step-form` wrapper instead of relying on a narrow shell, so header and stepper use the full width on all six steps. New `shared/components/bp-step-actions/` replaces six drifted per-step action bars with one three-zone bar — Atrás (left) · the step's own actions (projected centre) · Siguiente (right) — adopted by every step; step 6's forward action reads "Finalizar". "Volver al listado" hoisted from steps 1 and 3 into the wizard header, guarded by the shell's existing FR-007 unsaved-changes dialog (`pendingExit`), which retires both steps' own save-or-discard dialogs; "Guardar borrador" is now a first-class centre action on steps 1–3 rather than living only inside those dialogs. `import-step.component.ts` gains a `saved` output so step 4 advances to step 5 like every other step instead of dead-ending at "Ir al panel de organizador". Step 6: `table-board.component.ts` moves the tables into a `position: sticky` rail at the top (horizontally scrolled, "Add table" in its head) with the unassigned pool below as a `repeat(auto-fill, minmax(15rem, 1fr))` grid, plus a filter toolbar (blind code/style search, style and competition-category selects) that narrows the pool presentationally without touching assignments; `mesa-card.component.ts` gains a `compact` variant whose seated zones cap height and scroll internally so the rail's height stays fixed. E2E: `us4-judges.spec.ts`/`us14-judge-import.spec.ts` updated to "Siguiente". **Found but not fixed**: `us2-wizard.spec.ts`, `us3-import.spec.ts`, `us3-import-scale.spec.ts` and `e2e/a11y/routes.a11y.spec.ts` still address the wizard through pre-translation English labels (`Next`, `Save Draft`, `Upload`, `Consolidate`, `getByLabel('Name')`, heading `Import beer entries`) and a standalone `/import` route — they have been stale since the wizard was translated and restructured, independently of this task, and rewriting them blind without a live stack to run them against would be guesswork. Tracked as follow-up. Jest specs alongside for `bp-step-actions` and every touched step
- [X] T125b [FR-016] Step-6 layout refinement after review of T125 on a real screen. Root cause of "todo pegado": `--spacing-16` was referenced by `.wizard-main`'s `padding` shorthand but never defined in `styles.css`, so the whole declaration was invalid at computed-value time and the wizard rendered with **no padding at all**; token defined, plus gutters that widen at 1280px/1800px and a container capped at 88rem instead of 96rem. Board order inverted: unassigned pool (with its toolbar) on top, tables rail underneath — the pool's beer grid is capped at `40vh` and scrolls internally so it can never push the rail off screen, and the judges list at `12rem` for the same reason. Rail cards narrowed 17rem → 13rem with smaller type and `5.5rem` scrolling zones; `judge-seat.component.ts` gains a `dense` input (avatar and one-line name in a row) used by compact `mesa-card`s — the stacked avatar-over-name form was what made the cards tall, and dropping the name outright would have undone T124's whole point. "Add table" moved out of its own header row into a dashed tile inside the rail's horizontal scroller, so it costs zero vertical space while keeping both controls rendered and labelled for the nine E2E specs that address them. New "Ordenar por" control (estilo — the default, since balancing a table means spreading styles — categoría, graduación, código ciego); sorting is separate from the filters and `Limpiar filtros` deliberately leaves it alone
- [X] T127 [FR-061] Organizer page shell + wizard navigable (and read-only from `InEvaluation`) in every state. Spec amended first (Session 2026-09-19 clarification + new FR-061). Root cause of "the tables screen is glued to the edges with no topbar and no sense of where I am": no loose organizer route had a shell — `TableManagementComponent` was a bare `<app-table-board>`, and the dashboard, judge management, monitor and results dispatch likewise never used `bp-topbar`; only the wizard had margins. New `shared/components/bp-page-shell/` (topbar + single `<main>` + `88rem` container with the wizard's 1280px/1800px gutters, existing tokens only) adopted by all six organizer screens, the wizard included (its duplicated gutter CSS removed). `competition-wizard.component.ts`: `?step=N` deep link (read once on construction when an `:id` exists, 1–6 else step 1, mirrored via `Location.replaceState` so F5 no longer resets to step 1); `readOnly = computed(state === 'InEvaluation' || 'Finalized')` — FR-006's gate, `Active` stays editable (locking it was considered and dropped: the UI would forbid what the API accepts and break the table-assignment flow used in that state) — shown as a "Modo consulta" `bp-alert`, every step seeded as visited, and passed as `[readOnly]` to all six steps. Steps keep displaying all their data but hide/disable mutations: forms and selects disabled, "Guardar borrador"/category edit actions/dropzone/"Subir archivo"/row edit-exclude/"Consolidar" hidden, "Siguiente" re-emits the forward output without an API call; step 5 lists registered judges (existing `GET /competitions/{id}/judges`) instead of the upload phase; step 6 threads `readOnly` through `table-board` → `mesa-card`/`unassigned-column` (`cdkDropListDisabled`) → `beer-token`/`judge-seat` (new `dragDisabled` → `cdkDragDisabled`) and `table-detail-modal` (no "Move"), hiding the "Add table" tile; filters/sort/click-to-detail stay live. The standalone `/tables` route derives the same `readOnly` from its own `getById` (board rendered immediately, editable until known). Dashboard `destination()`: `Draft` → wizard, `Active` → wizard `?step=6`, `InEvaluation`/`Finalized` → monitor, which gains a "Ver configuración" link to the (read-only) wizard. `JudgeManagementApiService` relocated `features/judge-management/` → `core/api/` now that wizard step 5 is a second consumer (FSD, same precedent as T102's `CompetitionsApiService`). All E2E-locked board contracts unchanged. Jest 644 → 715 (58 suites), lint/format/tsc green, initial bundle 233.62 KB gzip. E2E run against a live Aspire stack: `us14-judge-import` green; the other eleven dependent specs fail on pre-existing debt unrelated to this change (the `createCompetition` helpers' pre-translation `getByLabel('Name')`, and `us4-judges` expecting "Notificar jueces" where `main` already renders "Notificar {n} jueces") — tracked for T128 together with the `us2`/`us3`/`a11y` debt noted under T125. PR #37 `senior-code-reviewer` pass (COMMENT, not a blocker) raised three pre-merge suggestions, addressed same task: `us13-dashboard.spec.ts` still expected `Active` → the old `/tables` route (M1, fixed to `?step=6` + the `Mesas` heading); read-only's step pre-seeding made every never-opened step's marker show a false amber "partial" instead of unmarked (M2, fixed with a `reportedSteps` set gating stepStatus on the step having actually mounted and emitted); and `onBasicsSaved` wrote the URL itself for a brand-new competition in the same tick as the `?step=` effect, racing it and briefly exposing a query-less URL that the `split('/').pop()` E2E helpers could read mid-flight (m1, fixed by making the effect the wizard's sole URL writer, preserving `Location`'s existing state, and making `support/organizer-wizard.ts` + the `us4`–`us14` helpers accept an optional `?step=N`). Jest 715 → 716 (58 suites); minors m2–m9/n1–n7 left open, not blockers per the review verdict.

...
- [X] T134 [FR-064] Decouple image deployment from infrastructure (option C, user decision 2026-09-26): Terraform takes per-image references (`api_image`/`web_image`/`keycloak_image`, replacing `image_tag`) used only when a Container App is first created (`lifecycle.ignore_changes` on the container image); `infra/deploy.ps1` stops building/pushing, accepts an optional version per image (`-ApiVersion`/`-WebVersion`/`-KeycloakVersion`: `X.Y.Z` → `<ns>/birrapoint-*-release:X.Y.Z`, omitted → `<ns>/birrapoint-*:latest`), verifies the images exist on Docker Hub, runs `terraform apply`, then rolls each app to its image with `az containerapp update` (keycloak → api → web, waiting for a healthy revision); `-AppsOnly` skips state bootstrap and Terraform (no tfvars, Neon key or state access) for the deploy pipeline; Pester tests for the image resolution logic

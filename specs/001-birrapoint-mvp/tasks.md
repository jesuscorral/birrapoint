...
- [X] T133 [FR-064] CI pipeline: `version.txt` (initial `0.1.0`) and `.github/workflows/ci.yml` — PR runs backend/frontend quality gates plus non-pushing `docker build` of api/web/keycloak; push to `main` additionally publishes `latest` and `sha-<short>` images to Docker Hub (serialized with a `concurrency` group; keycloak rebuilt only when `infra/keycloak/**` changes)

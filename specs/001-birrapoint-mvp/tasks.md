...
- [ ] T136 [FR-064] Deploy pipeline `.github/workflows/deploy.yml` (`workflow_dispatch`, optional per-image version inputs): `production` Environment with required reviewers, Azure OIDC login scoped to the application resource group, runs `infra/deploy.ps1 -AppsOnly` with the chosen versions, deploy `concurrency` group; documents Azure OIDC/Docker Hub/GitHub App setup

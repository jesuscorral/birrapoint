...
- [ ] T138 Integration-test flakiness (PR #48 review L5): one `BirraPoint.Api.IntegrationTests` failure seen locally during T133 did not reproduce on re-run and is now a CI merge gate — identified in CI on PR #50 (2026-09-26): `DispatchApiTests.Retry_dispatch_resets_a_failed_job_and_it_gets_reprocessed_by_the_safety_net_poll` (timing-dependent wait on the DispatchWorker safety-net poll); confirm it is the only one and fix its root cause (no retries masking it)
